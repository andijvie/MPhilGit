...
# ----------------------2 group Discrete ordinates transport function----------------------
# - this function performs the neutron transport for exercise 7
# - it is based on the method outlined in the 1 group version, but it has 2 energy groups, it does not use the iterationConstants class, instead it relies on the constants above
# Inputs:
...
# - fluxGuess1: the initial guess for phi in group 1 [FLOAT ARRAY]
# - fluxGuess2: the initial guess for phi in group 2 [FLOAT ARRAY]
...
# - float array of the phi found for group 1
# - float array of the phi found for group 2
def solveDiscreteOrdinates_2group(
    ...
    QNext1 = np.array([]) # stores the next source array group 1
    QNext2 = np.array([]) # stores the next source array group 2
    ...
    def getPsi(abscissa, QNext, slab_macro_t):
        ...
        psi[0] = .0 # start with 0 on either side
        ...
        for it in range(0, mesh_intervals):
            ...
            if psi_int < 0:
                print("WARNING in psi sweep: less than 0!") # this warning was never observed.
    ...
    def hasConverged(): 
        ...
        print("loss group 1 = " + str(loss1) + ", group 2 = " + str(loss2)) # print the losses
    ...
    while not hasConverged():
        ...
        fluxNext1 = np.zeros(mesh_points)[:, np.newaxis]  # initialize the neutron flux, start with all 0s, group 1
        fluxNext2 = np.zeros(mesh_points)[:, np.newaxis]  # initialize the neutron flux, start with all 0s, group 2
        for i in range(order): # iterate over the angles

            # calculate the angular fluxes
            angFlux1 = getPsi(abscissae[i], QNext1, slab_macro_t_1)[:, np.newaxis]
            angFlux2 = getPsi(abscissae[i], QNext2, slab_macro_t_2)[:, np.newaxis]

            # add the angular fluxes to the scalar fluxes
