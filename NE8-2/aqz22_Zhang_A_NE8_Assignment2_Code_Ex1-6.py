...
class IterationConstants:
    def __init__(self,
                    ...
                    is_isotropic = False, # For assignment 1, exercise 5: This boolean changes the scattering to isotromic by setting the average scattering angle mu_bar to 0. 
                    use_scatter_ratio = False, # For assignment 2, exercise 6: This boolean enables the scatter ratio c (see below)
                    scatter_ratio = 0.1, # c = Sigma_s_tr/Sigma_tr, when use_scatter_ratio is enabled, this value changes Sigma_s_tr with respect to Sigma_tr (the latter stays constant)
                    multiplying = True, # For assignment 2, exercise 6: This boolean disables fissions when set to false
        ...
        self.anal_x_axis = np.linspace(-100/2, 100/2, self.mesh_points) # [cm] analytic x axis, since the analytic solution must have a slab length of 100 cm
        self.anal_delta_x = 100/self.mesh_intervals # [cm] analytic delta x
        self.L = np.sqrt(self.diffusion_coefficient / ((self.slab_macro_f_nu/self.kAnal) - self.slab_macro_a)) # The L quantity in flux=Acos(x/L)
        self.fluxAnal = np.cos(self.anal_x_axis /self.L) # [cm-2s-1] analytic flux
        self.fluxAnal *= 1e3 / integrate(self.slab_macro_f_pow * self.fluxAnal, self.anal_delta_x) # normalize to 1000 W/cm2

        # Disable fissions if necessary
        if not multiplying: 
            self.slab_macro_f_nu = 0
            self.macro_f_nu_matrix = product_operator(self.mesh_points, self.delta_x, np.full(self.mesh_intervals, self.slab_macro_f_nu)) # matrix to perform the fnu multiplication 
        
        # Apply scatter ratio when necessary
    ...
    # - Compares the outcome of a neutron transport computation to either: the analytic solution, or another neutron transport outcome  
    # Inputs:
    # - keffIn: eigenvalue found [FLOAT]
    # - phiIn: the neutron flux found [FLOAT ARRAY]
    # - doPlot: plot the comparrison?
    # - reference: compare to a different solution? Default: False ---> compare to analytic solution [BOOLEAN]
    # - refKeff: other eigenvalue [FLOAT]
    # - refPhi: other neutron flux [FLOAT ARRAY]
    # Output:
    # - float of the absolute eigenvalue difference
    # - float of the maximum relative flux difference 
    def compare(self, keffIn, phiIn, doPlot = False, reference = False, refKeff = -9999.9, refPhi = np.array([-9999.9])):

        # load the analytical solution in necessary
        if not reference:
            refKeff = self.kAnal
            refPhi = self.fluxAnal
        
        # print the absolute and relative difference in keff
        print("Abs err keff = " + str(keffIn - refKeff) + " relative err = " + str((keffIn - refKeff)/refKeff))

        # find the value and location of the maximum relative and absolute differences in phi
        maxAbs = 0
        maxRel = 0
        maxAbsX = -1
        maxRelX = -1
        ...
        # print the value and location of the maximum relative and absolute differences in phi
        print("Max absolute flux err = " + str(maxAbs) + " @ " + str(maxAbsX) + ", max relative flux err = " + str(maxRel) + " @ " + str(maxRelX))
        
        # plot the different flux distributions in necessary
        if doPlot:
        ...
        # return the absolute difference in keff and max relative difference in phi
        return (keffIn - refKeff, maxRel)



# ----------------------Power iteration function----------------------
# - this function performs the diffusion iteration 
...
# - doSave: if the flux should be saved to a file (handy for plotting) [BOOLEAN]
...
# ----------------------Discrete ordinates transport function----------------------
# - this function performs the neutron transport for exercises 1-6
# - it is based on the method outlined in the handout
# - the function has many inputs to account for the different exercises
# Inputs:
# - convergenceCriteria: selects the convergence criteria, 0 is the one based on k, 1 is the one based on phi [0 OR 1]
# - order: the order of the quadrature [INT, EVEN]
# - fluxGuess: the initial guess for phi [FLOAT ARRAY]
# - eigenGuess: the initial guess for keff [FLOAT]
# - doPlot: if plots should be made [BOOLEAN]
# - doSave: if the flux should be saved to a file (handy for plotting) [BOOLEAN]
...
# - powerTarget: the value of power that the flux should be normalized to [FLOAT, W/cm2]
# - ic: the iteration constants containing the matrices, this is an input to reduce computation time when solving multiple times [ITERATIIONCONSTANTS OBJECT]
# - useRectangle: if the quadrature should be switched to a uniform quadrature of equally sized rectangles (default False ---> Gauss legendre quadrature) [BOOLEAN]
# - doSavePolar: for exercise 4, if the angular fluxes need to be saved [BOOLEAN]
# - polarIt: the iterations after which all angular fluxes start to be saved, this should ideally be close to the final iteration to save time [INT]
# - closure: selects the close, 0 is for the Diamond-difference closure, 1 is for the step closure [0 OR 1]
# - leftSource: for exercise 6, selects the source strength for a source on the left of the slab, default is 0 equivalent to no source [FLOAT]
# Outputs:
# - float of the keff found
# - integer of number of iterations needed
# - float array of the phi found
def solveDiscreteOrdinates(
    ...
    eigenNext = eigenGuess # keff_n+1
    eigen = None # keff_n
    abscissae, weights = leggauss(order) # stores the Gauss-Legendre quadrature
    if useRectangle: # if necessary, change to a uniform quadrature
        weights = np.full(order, 2/order)
        abscissae = np.linspace(-1, 1, order, endpoint=False) + 1 / order
    QNext = np.array([]) # stores the next source array
    eigenHist = np.array([eigenNext]) # stores the histary of keff iterations
    fluxHist = np.empty((ic.mesh_points, 0)) # stores the history of phi iterations
    fluxHist = np.append(fluxHist, fluxNext, axis=1) # add first guess
    loss = -1 # stores the error in the convergence criteria
    ...
    # The S vector is a holdover from the diffusion algorithm, it just stores the value of ic.macro_f_nu_matrix @ fluxNext for the eigenvalue calculation
    S = np.array([]) # S_n
    SNext = ic.macro_f_nu_matrix @ fluxNext # S_n+1

    # performs the angular flux sweeps 
    def getPsi(abscissa):
        ...
        coeff = 2 if closure == 0 else 1 # the main difference between the 2 closure relations is that a certain coefficient is either 1 or 2, this variable stores that
        psi[0] = .0 if abscissa < 0 else leftSource # enable the left source if necessary

        # loop that performs the sweep
        psiPrev = 0 # stores the previous value of the angular flux
        for it in range(0, ic.mesh_intervals):

            # the logic below determines the index of the array depending on if were sweeping towards the left or the right
            if abscissa > 0:
                iNow = it 
                iNext = it + 1
            else:
            ...
            # sweep over psi depending on the closure relation
            psi_int = (ic.delta_x * QNext[iNow] + coeff * np.abs(abscissa) * psiPrev) / (
                                ic.delta_x * ic.slab_macro_tr + coeff * np.abs(abscissa)
                            )
            
            # it is possible for the diamond-difference closure to end up with a (non-physical) negative flux, in such cases, issue a warning
            if psi_int < 0:
                print("WARNING in psi sweep: less than 0!") # this warning was never observed except when the scattering ratio was set to a non-physical quantity

            # set the next psi depending on the closure relation
    ...
    while not hasConverged():
        ...
        # calculate n+1 values of psi, phi and Keff
        QNext = interpolate((1/2) * (ic.slab_macro_s_tr + ic.slab_macro_f_nu/(eigen + 1e-99)) * flux) # calculate the source vector
        fluxNext = np.zeros(ic.mesh_points)[:, np.newaxis] # initialize the neutron flux, start with all 0s
        for i in range(order): # iterate over the angles
            angFlux = getPsi(abscissae[i])[:, np.newaxis] # calculate the angular flux
            fluxNext += weights[i] * angFlux # add the angular flux to the scalar flux

            # save the angular flux if necessary
...
# runs the basic diffusion solver
# this code is always run as a sanity check to see if matplotlib has loaded correctly
...
# This function solves the discrete ordinates neutron transport
# It is a seperate function so lineProfiler can be used to examine the run time
...
# UNCOMMENT the lines below to use lineprofiller to examine the run time
#lp = LineProfiler()
#lp.add_function(lpf)
...
#q1()  # <--- UNCOMMENT TO RUN QUESTION 1
...
def q2():
    ...
    # generate the constatns used for the neutron transport with a slab length of 30 cm
    ic30 = IterationConstants(resolution=10, slab_thickness = 30, is_isotropic=False, doPrint=False)

    # quadratrures to be examined
    ordersRect = np.array([2, 4, 6, 8, 10, 12]) # uniform quadratures order 2-12
    ordersLeg = np.array([2, 4, 6, 8, 10, 12]) # Gauss-Legendre quadratures order 2-12

    # Loop 1: Gauss-legendre quadratures
    for order in ordersLeg:

        # Solve neutron transport and store the outcome
    ...
    # Plot 1: eigenvalue convergences
    plt.figure(figsize=((8,3)))
    ...
    # Plot 2: eigenvalue error with respence to S_12
    plt.figure(figsize=((8,3)))
...
#q2()  # <--- UNCOMMENT TO RUN QUESTION 2
...
# function that runs question 3
def q3():
    lengths = np.linspace(5, 500, 25) # <-- lengths that are computed
    #lengths = [100] # <-- UNCOMMENT for extra 100 cm plot
    kErrHist = np.array([]) # list of k differences
    phiErrHist = np.array([]) # list of relative phi errors
    itHistDO = np.array([]) # list of discrete ordinates iteration numbers
    itHistDiff = np.array([]) # list of diffusion iteration numbers

    # Main loop for all the lenths
    for l in lengths:

        # Generate the constants for the neutron transport
        icRes = IterationConstants(resolution=100/l, slab_thickness = l, is_isotropic=False, doPrint=True)
        print("Slab Length = " + str(l) + " cm")

        # Solve neutron transport using both methods
        ...
        # Calculate and store all the relavent quantities
        kErr, phiErr = icRes.compare(keffDO, phiDO, reference=True, refKeff=keffDiff, refPhi=phiDiff, doPlot=l in [5, 100, 500]) # <-- plots are made for the 5, 100, 500 cm cases
    ...
    # plot the abs keff difference on a log scale
    plt.figure(figsize=((8,3)))
    plt.plot(lengths, np.abs(kErrHist), color='k', linestyle = '-', linewidth = '1.5', marker = 'o')
    plt.yscale('log')
    plt.ylabel(r"$|k_\text{eff}^\text{DO} - k_\text{eff}^\text{Diff}|$", fontsize=12)
    plt.xlabel(r"Slab length [cm]", fontsize=12)
    plt.tight_layout()
    ...
    # plot the max relative flux difference
    plt.figure(figsize=((8,3)))
    ...
    # plot the ratio
    plt.figure(figsize=((8,3)))
    plt.plot(lengths, itHistDO/itHistDiff, color='k', linestyle = '-', linewidth = '1.5', marker = 'o', label="Discrete ordinates")
    plt.ylabel(r"Iterations$_\text{DO}$ / Iterations$_\text{Diff}$", fontsize=12)
    plt.xlabel(r"Slab length [cm]", fontsize=12)
    plt.tight_layout()
    plt.show()


#q3() # <--- UNCOMMENT TO RUN QUESTION 3
...
def q4():
    # solve the discrete ordinates and store the angular fluxes
    solveDiscreteOrdinates(convergenceCriteria = 1, order=12, doPlot=False, ic=icBase, doSavePolar=True, polarIt=1000)

    angFluxStart = np.array([]) # stores the left fluxes
    angFluxMid = np.array([]) # stores the middle fluxes
    angFluxEnd = np.array([]) # stores the right fluxes
    abscissae, _ = leggauss(12) # S_12 abscissae used to find the agnled
    angles = np.arccos(abscissae) # the inverse cosine gives the angles corresponding to the abscissae

    # load the angular fluxes
    for i in range(12):
    ...
    # append the negative angles of the fluxes such that a whole circle can be plotted
    ...
    # append the original value so the rose plot starts and ends at the same point
    # devide my 2 pi so the unit becomes /radian
...
#q4() # <--- UNCOMMENT TO RUN QUESTION 4
...
def q5():
    ...
    # res = np.arange(1, 5.1, 0.1) # [points/cm] UNCOMMENT for additional resolutions to be checked
    ...
    _, fluxErrFinD = ic.compare(eigenRes, flux, doPlot=False) # fluxErrFinD = 0.18642349969925093 # @ 50 
    ...
    _, fluxErrFinS = ic.compare(eigenRes, flux, doPlot=False) # fluxErrFinS = 0.18261859862294094 # @ 50
    print("ERROR = " + str(fluxErrFinS))
    print()

    # loop1: diamond-difference
    # initialize plot for phi
    plt.figure(figsize=((8,3)))
    plt.xlim(-51, 51)
    ...
    # plot the progression of the flux error
    plt.figure(figsize=((8,3)))
...
# q5() # <--- UNCOMMENT TO RUN QUESTION 5
...
def q6():
    ...
    # PART 1: plot the fluxes, and store the iterations

    # UNCOMMENT depending on which scatter ratios are to be plotted:
    scatter_ratios = [] # DEFAULT: skip part 1
    #scatter_ratios = [1.0e-03, 3.0e-03, 5.0e-03, 1.0e-02, 3.0e-02, 5.0e-02] # Plot 1, 2 ratios
    #scatter_ratios = [0.1, 0.3, 0.5, 0.9, 0.93, 0.95] # Plot 3 ratios
    #scatter_ratios = [0.99, 0.992, 0.994, 0.996, 0.997, 0.998] # Plot 4 ratios
    #scatter_ratios = [0.999, 0.9993, 0.9995, 0.9999, 0.99999, 1] # Plot 5 ratios
    ...
    itHist = np.array([]) # stores the number of iterations for the iterations plot
    plt.figure(figsize=((8,3))) # initialize the flux plot

    # main loop over the scatter ratios
    for i in range(len(scatter_ratios)):

        # generate the constants for the neutron transport using the scatter ratio
        c = scatter_ratios[i]
        icC = IterationConstants(use_scatter_ratio=True, scatter_ratio=c, multiplying=False)

        # solve the neutron transport and save the results
        ...
        # Plot flux,
        # Uncomment x and y limits depending on the plot:
        #plt.xlim(-50, -46) # 2
        plt.xlim(icC.x_axis[0], icC.x_axis[-1]) # Rest
        #plt.ylim(-2e-4 * 0.1, 2e-4) # 1
        #plt.ylim(-1e-3 * 0.1, 1e-3) # 2
        #plt.ylim(-0.2e-1 * 0.1, 0.2e-1) # 3
        #plt.ylim(-0.04 * 0.1, 0.04) # 4
        plt.ylim(-0.05 * 0.1, 0.05) # 5

        plt.plot(icC.x_axis, flux, linestyle = linestyle[i], color = color[i], linewidth = 1, label=f'c={c}')

    # finalize the flux plot
    plt.ticklabel_format(axis='y', style='sci', scilimits=(0,0))
    plt.xlabel(r"Position, $x$ [cm]", fontsize=12)
    plt.ylabel(r"Scalar Flux, $\phi$ [cm$^{-2}$s$^{-1}$]", fontsize=12)
    plt.legend(ncol=1,
    ...
    # save the number of iterations required
    # UNCOMMENT depending on which plot needs to be saved
    ...
    # PART 2: plot the iterations
    scatter_ratios = np.array([]) # stores the scatter ratio c
    itHist = np.array([]) # stores the number of iterations required
    ...
    # order to increasing scatter ratio
    order = np.argsort(scatter_ratios)
    scatter_ratios = scatter_ratios[order]
    itHist = itHist[order]

    # plot the iterations required
    plt.figure(figsize=((8,3)))
    plt.plot(scatter_ratios, itHist, color='k', linestyle = '-', linewidth = '1.5', marker = 'o')
    plt.ylabel("Iterations", fontsize=12)
    plt.xlabel(r"Scattering Ratio, $c$", fontsize=12)
    plt.tight_layout()
    plt.show()

#q6()  # <--- UNCOMMENT TO RUN QUESTION 6


# ----------------------Exercise 7----------------------
# see: aqz22_Zhang_A_NE8_Assignment2_Code_Ex7.py
